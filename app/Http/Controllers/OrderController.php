...
class OrderController extends Controller
{
    ...
    public function joinDriver(FoodOrderDataTable $foodOrderDataTable, $id)
    {
        $this->orderRepository->pushCriteria(new OrdersOfUserCriteria(auth()->id()));
        //$this->orderRepository->leftJoin('drivers', 'order.driver_id', '=', 'drivers.id')->get()->pushCriteria(new OrdersOfUserCriteria(auth()->id()));
    ...
    public function update($id, UpdateOrderRequest $request)
    {
        ...
        $oldStatus = isset($oldOrder->payment->status) ? $oldOrder->payment->status : 0;
        $input = $request->all();
        $customFields = $this->customFieldRepository->findByField('custom_field_model', $this->orderRepository->model());
        try {
            ...
            if(isset($order->delivery_address_id) && isset($order->restaurant_id))
            {
