...
function getCustomFieldsValues($customFields = null, $request = null)
{
    ...
    $validateVehicle = array_search('Tipo de vehiculo', array_column($customFields->toArray(), 'name'));
    if ($validateVehicle == false)
        $request['Tipo_de_vehiculo'] = 'auto';
    
    foreach ($customFields as $cf) {

        $cf->name= $cf->name == "Tipo de vehiculo" ? "Tipo_de_vehiculo" : $cf->name;
        $value = $request->input($cf->name);
        if ($cf->id == 7 && $request->input($cf->name) == "")
            $value = 'auto';
        ...
        if ($fieldType === 'selects') {
            ...
            $view = isset($value) ? array_flip($view)[$value] : null;
...
function getSpecialStatus($column, $attributeName)
{
    if (isset($column)) {
...
function calculateDistance($point1_lat, $point1_long, $point2_lat, $point2_long, $decimals = 2) {
    if ($point1_lat == 0 || $point1_long == 0 || $point2_lat == 0 || $point2_long == 0)
        return null;
    ...
    $resultDistance = ( (60 / 100) * ( round($distance, $decimals) / 30 ) ) * 100;
    return (int) $resultDistance + 20;
}