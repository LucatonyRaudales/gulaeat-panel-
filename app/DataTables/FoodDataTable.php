...
class FoodDataTable extends DataTable
{
    ...
    public function dataTable($query)
    {
        ...
        $dataTable = $dataTable
            ...
            ->editColumn('status', function ($food) {
                return getSpecialStatus($food, 'status');
            })
    ...
    public function query(Food $model)
    {
        ...
        if (auth()->user()->hasRole('admin')) {
            return $model->newQuery()->with("restaurant")->with("category")->select('foods.*')->orderBy('foods.status','asc');
            ...
            return $model->newQuery()->with("restaurant")->with("category")
                ...
                ->groupBy('foods.id')
                ->select('foods.*')->orderBy('foods.status', 'asc');
